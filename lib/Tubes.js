...
var lodr, lodr1, lodr2, lodr3, lodr4;                                    //    objek
var txt;                                                         //     terrain
var player = { height:1.8, speed:0.2, turnSpeed:Math.PI*0.02 }; //      camera seakan fps tanpa first person control
...
function init(){
    ...
    // scene.fog = new THREE.Fog(fogColor, 0.0021, 20);
    ...
    lodr3 = new THREE.Mesh(

    );

    lodr4 = new THREE.MTLLoader();
    lodr4.load("Tree/Tree.mtl", function(object){

        object.preload();
        objload = new THREE.OBJLoader();
        objload.setMaterials(object);

        objload.load("Tree/Tree.obj", function(mesh){
            scene.add(mesh);
